...
import { View, ScrollView, Pressable, TextInput, KeyboardAvoidingView, Platform, Dimensions } from 'react-native'
...
export function TutorChat({
  ...
  const handleSend = useCallback(async () => {
    ...
    setTimeout(() => {
      const aiMessage: ChatMessage = {
        id: `msg-${Date.now() + 1}`,
        role: 'assistant',
        content: `Đây là câu trả lời mẫu cho: "${userMessage.content}". Trong phiên bản thực tế, AI sẽ phân tích nội dung bài học và đưa ra câu trả lời phù hợp dựa trên ngữ cảnh.`,
        created_at: new Date().toISOString(),
        status: 'sent',
      }
  ...
  return (
    <Animated.View
      ...
      className={`absolute right-0 bottom-0 w-full rounded-t-3xl shadow-2xl z-50 ${isDark ? 'bg-zinc-900' : 'bg-white'}`}
